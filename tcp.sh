...
installbbr(){
	...
	stty erase '^H' && read -p "You need to restart the VPS before you can start the BBR/BBR magic revision, whether to restart now? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
...
startbbrmod(){
	...
	sysctl -p
    cd .. && rm -rf bbrmod
	echo -e "${Info}The Magic Revised BBR started successfully功！"
}
...
startbbrmod_nanqinlang(){
	...
	sysctl -p
	echo -e "${Info}The magic revamped BBR started successfully！"
}
...
Update_Shell(){
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		echo -e "New version found[ ${sh_new_ver} ]，Update？[Y/n]"
		read -p "(default: y):" yn
		[[ -z "${yn}" ]] && yn="y"
		if [[ ${yn} == [Yy] ]]; then
...
————————————————————————————————" && echo
	...
	if [[ ${kernel_status} == "noinstall" ]]; then
		...
		echo -e " Current state: ${Green_font_prefix}Installed${Font_color_suffix} ${_font_prefix}${kernel_status}${Font_color_suffix} Speed up the kernel , ${Green_font_prefix}${run_status}${Font_color_suffix}"
...
detele_kernel(){
	if [[ "${release}" == "centos" ]]; then
		...
		if [ "${rpm_total}" > "1" ]; then
			echo -e "detected ${rpm_total} The remaining cores, start to uninstall..."
			for((integer = 1; integer <= ${rpm_total}; integer++)); do
				rpm_del=`rpm -qa | grep kernel | grep -v "${kernel_version}" | grep -v "noarch" | head -${integer}`
				echo -e "Start uninstall ${rpm_del} Kernel..."
				rpm --nodeps -e ${rpm_del}
				echo -e "Uninstall ${rpm_del} Kernel uninstall is complete, continue..."
			done
			echo --nodeps -e "After the kernel is unloaded, continue..."
		else
		...
		if [ "${deb_total}" > "1" ]; then
			echo -e "detected ${deb_total} The remaining cores, start to uninstall..."
			for((integer = 1; integer <= ${deb_total}; integer++)); do
				deb_del=`dpkg -l|grep linux-image | awk '{print $2}' | grep -v "${kernel_version}" | head -${integer}`
				echo -e "Start uninstall ${deb_del} Kernel..."
				apt-get purge -y ${deb_del}
				echo -e "Uninstall ${deb_del} Kernel uninstall is complete, continue..."
			done
			echo -e "After the kernel is unloaded, continue..."
		else
			echo -e " Incorrect number of cores detected, please check !" && exit 1
		fi
	fi
	...
	if [[ ${kernel_status} == "Lotserver" ]]; then
		...
		if [[ ${run_status} == "bbr" ]]; then
			...
			if [[ ${run_status} == "tcp_nanqinlang" ]]; then
				...
				run_status="Violent BBR Magic revamp failed to start"
			fi
		else 
			run_status="No acceleration module installed"
		fi
